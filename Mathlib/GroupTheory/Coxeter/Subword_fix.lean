...
private def self_mem_expSet {l l' : List (indexOf W)} (hl : IsReduced (csOf W) l)
  (hsub : l <+ l') : expSet W l l' := ⟨l, ⟨rfl, ⟨hl, hsub⟩⟩ ⟩

private instance (l l' : List (indexOf W)) : Fintype (expSet W l l') := by
  sorry

lemma le_of_subword_aux (hl : IsReduced (csOf W) l) (hl' : IsReduced (csOf W) l') (hsub : l <+ l')
  (h : ∀ ll : expSet W l l', ¬ll.1 <:+: l')
    : ∃ ll, IsReduced (csOf W) ll ∧ ll <+ l' ∧ π l < π ll ∧ ll.length = l.length + 1 := by
      classical
      let f (ll : expSet W l l') : ℕ := first_deleted_index (h ll)
...
lemma le_of_subword  (hl : IsReduced (csOf W) l) (hl' : IsReduced (csOf W) l') (hsub : l <+ l') :
  π l ≤ π l' := by
    by_cases h : ∃ ll : expSet W l l', ll.1 <:+: l'
    · obtain ⟨ll, hll⟩ := h
      exact ll.2.1 ▸ le_of_infix W ll.2.2.1 hl' hll
    · push_neg at h
      ...
      · let ll := Classical.choose <| le_of_subword_aux W hl hl' hsub h
        let ll_spec := Classical.choose_spec <| le_of_subword_aux W hl hl' hsub h
        by_cases hll : ∀ lll : expSet W ll l', ¬lll.1 <:+: l'
        ...
        · push_neg at hll
          let ll := (choose <| hll)
          have := le_of_infix W ll.2.2.1 hl' (choose_spec <| hll)
