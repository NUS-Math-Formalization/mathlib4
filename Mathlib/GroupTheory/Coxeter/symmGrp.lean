...
variable {B : Type}
variable {W G: Type} [Group W] [Group G]
...
variable {f : B → G} (hf : M.IsLiftable f)

lemma lift_inj : (cs.lift ⟨f, hf⟩ ).toFun.Injective := by
  intro u v
  simp
  revert v
  apply WellFounded.induction ((measure cs.length).wf) u
  -- apply WellFounded.induction ((measure cs.length).wf) v
  ...
  by_cases hu : u = 1
  · simp [hu] at h2
  -- induction' u using WellFounded.induction
  -- simp [lift]
    sorry
  sorry

-- variable (n : ℕ)
-- abbrev M := CoxeterMatrix.Aₙ n
-- abbrev cs := (M n).toCoxeterSystem
